...
func runDeviceCombinations(t *testing.T, devName string, keyDir string, drmType string) {
   ...
   // Restricted to 2.3 only as requested
   hdcpLevels := []string{"2.3"}
   resolutions := []string{"576p", "2160p"}
   hdrFormats := []string{"None", "HDR10", "DolbyVision"}
   // Added codec array as requested
      ...
      for _, res := range resolutions {
         for _, hdr := range hdrFormats {
            for _, codec := range codecs {
               ...
               comboName := fmt.Sprintf("HDCP:%s Res:%s HDR:%s Codec:%s", hdcp, res, hdr, codec)
               ...
               profile := DeviceProfile{
                  ...
                  MaxResolution: res,
                  HDRFormats:    hdr,
                  VideoCodec:    codec, // Pass codec into the profile
               }
                  ...
                  for _, adp := range period.AdaptationSets {
                     if adp.ContentType == "video" || adp.MimeType == "video/mp4" {
                        for _, rep := range adp.Representations {
                           repCopy := rep
                           // Now selecting the HIGHEST bandwidth instead of lowest
               ...
               fmt.Printf("[INFO] %s -> Selected highest bandwidth representation -> Width: %d, Height: %d, Codecs: %s (Bandwidth: %d)\n",
                  comboName, highestRep.Width, highestRep.Height, highestRep.Codecs, highestRep.Bandwidth)
