...
func SubmitOTP(client *http.Client, actionUrl string, hiddenParams map[string]string, otpCode string) (string, error) {
   data := url.Values{}
   for k, v := range hiddenParams {
      data.Set(k, v)
   }
   data.Set("code", otpCode)
   ...
   return "", fmt.Errorf("expected 302 redirect containing Claim Token, got %d", resp.StatusCode)
}
