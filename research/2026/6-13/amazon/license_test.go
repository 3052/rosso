...
func TestGetWidevineLicense(t *testing.T) {
   ...
   // Passing nil for the challenge as requested for now
   license, err := GetWidevineLicense(actorState.AccessToken, titleId, envState.PlaybackEnvelope, nil)
   if err != nil {
      t.Fatalf("Failed to get Widevine license: %v", err)
   }

   t.Logf("Successfully retrieved Widevine license! (Length: %d bytes)", len(license))
}
