...
4. Unmarshal JSON responses into domain-specific struct pointers. When parsing a JSON response body, use `json.NewDecoder(resp.Body).Decode(...)` directly. Do not read the entire response body into a byte slice using `io.ReadAll` prior to unmarshaling. If you must read the body into memory using `io.ReadAll` (e.g., for returning raw binary responses), reuse the existing `body` variable; do not declare a new `respBody` variable.
...
19. Always align variable and parameter names with standard library conventions and function signatures. Use `resp` instead of `res` for HTTP responses. When serializing a payload to pass as the body parameter of a request function, name the resulting byte slice variable `body` instead of `data` or `buf` to match the function signature. If constructing a struct before serialization, name the struct variable something else so the serialized byte slice can utilize the `body` identifier. When declaring a variable, parameter, or loop variable whose type is exactly the single-word base entity (ignoring pointers), you must append `Data` to the name (e.g., `entityData Entity` or `entityData *Entity`) to avoid a case-insensitive match with the type. Do not carry over secondary descriptive words from the type name into the variable or parameter name (e.g., do not use `entitySessionData *EntitySession`), and do not use stuttering or repetitively suffixed names.
