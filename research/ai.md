...
13. Variable, Parameter, Receiver, and Loop Variable naming: YOU MUST NEVER USE THE EXACT SAME STRING (IGNORING CASE) FOR BOTH THE IDENTIFIER AND ITS TYPE. 
    ...
    * For TWO-WORD Types (e.g., `ItemDetails`, `AccessKey`): The variable name MUST be a SINGLE word extracted directly from the type name to prevent stuttering (e.g., `item ItemDetails`, `details ItemDetails`, `key AccessKey`). NEVER add arbitrary descriptors (e.g., `inputKey AccessKey`) and NEVER use the exact type name.
    * Exceptions: If a type ends in a generic suffix indicating a response or payload, the variable MUST be named `resp` or `payload`.
