...
19. ALWAYS align variable and parameter names with standard library conventions, EXCEPT YOU MUST NEVER USE SINGLE-LETTER VARIABLES. Use descriptive one-word (or two-word if ambiguous) names. Use resp for HTTP responses and query or values for URL query maps. When serializing a payload, name the resulting byte slice variable identically to the function signature parameter. If constructing a struct before serialization, name the struct variable something else so it can use the parameter identifier. When declaring a variable, parameter, receiver, or loop variable whose type shares its base entity, you MUST adhere strictly to the naming structures defined in Rule 13 to avoid case-insensitive collisions. NEVER carry over secondary descriptive words from the type name into the variable or parameter name unless dictated by the aforementioned forms, and NEVER use stuttering or repetitively suffixed names.
