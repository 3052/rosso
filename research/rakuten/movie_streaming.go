...
type MovieStreamingInfo struct {
   Url  string        `json:"url"`
   Wrid StreamingUuid `json:"wrid"`
}

type MovieStreaming struct {
   StreamInfos []MovieStreamingInfo `json:"stream_infos"`
}

type MovieStreamingPayload struct {
...
func CreateMovieStreaming(session *UserSession, movie *FeatureMovie) (*MovieStreaming, error) {
   endpoint := &url.URL{
      Scheme: "https",
