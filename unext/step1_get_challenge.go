...
// Package-internal PKCE state. Generated by Step1GetChallenge,
// consumed by Step3GetAuthCode and Step4GetToken.
var (
   pkceVerifier  string
...
// pair is stored in package-level variables for use by Step3 and Step4.
func Step1GetChallenge() (string, error) {
   state, err := generateRandomString(32)
   if err != nil {
      return "", fmt.Errorf("step1: generating state: %w", err)
   }
   nonce, err := generateRandomString(32)
   if err != nil {
      return "", fmt.Errorf("step1: generating nonce: %w", err)
   }
   pkceVerifier, pkceChallenge, err = pkcePair()
   if err != nil {
      return "", fmt.Errorf("step1: generating PKCE pair: %w", err)
   }
